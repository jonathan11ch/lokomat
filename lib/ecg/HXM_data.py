...
class HXM_data(object):
	...
	#Funcion para extraer los datos del fiche
        time.sleep(5)
        while self.acquire_on:
            time.sleep(self.time_adq)
            self.f1=open(self.__name_csv,'r')
            print self.f1.readlines()[-1]
            self.f1.close()

    def save_data(self):
	#Funcion que adquiere continuamente los datos del sensor y los almacena en un fichero
        while self.acquire_on:
        	d = self.__ser.read()
        	if d != self.__stx:
        		if not self.__async:
